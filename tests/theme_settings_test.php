...
class theme_edpearson_site_logo_testcase extends advanced_testcase {
    public function test_theme_settings() {
        global $CFG, $SITE, $OUTPUT, $PAGE;
        $PAGE->theme->settings->logo = new moodle_url('/theme/image.php/_s/edpearson/totara_core/1/logo');
        $logoalt = 'Theme Logo';
        $this->resetAfterTest();
        $expected = array(
            'siteurl' => $CFG->wwwroot .'/',
